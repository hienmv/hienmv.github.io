...
- Max-heap: mỗi node cha đều có giá trị lớin hơn hoặc bằng node con của nó. 
...
1. Với 1 binomial heap có n nodes:
   ...
   1. Binamial heap phải thoả mãn các tính chất sau: (Binomial Heap Property)
...
### Một số binomial heap functionalities cơ bản. 

1. find-min: tìm min-value trong heap O(logn)

   1. Có nhiều nhất log2(n) + 1 binamial trees trong heap, và duyệt qua từng root node của mỗi tree đó để tìm min-value. 
...
![](../assets/BinominalHeapExample3.png)
   3. Merge binomial tree pseudo code:
...
1. extract-min : xoá node chứa min-value O(logn)

   1. Sử dụng find-minđể tìm ra root node root_node trong danh sách root và xoá node này đi. 
...
2. insert thêm một phần tử vào heap H: amortized O(1)

   1. Tạo ra 1 heap mới H'với phần tử muốn thêm vào heap ban đầu.
   2. Merge H và H'

3. decrease value lấy phần tử xx trong binomial heap và gán value = v. O(logn)
...
4. remove xoá phần tử xx ra khỏi heap. O(logn)

   1. Gán value của xx là 1 số âm, sử dụng decrease value
   2. Sử dụng extract-min

5. [sample code](https://www.nayuki.io/res/binomial-heap/binomialheap.py)
...
2. Fibonacci heap tương tự như binomial heap, nhưng sử dụng cơ chế lazy - chỉ merge trees khi functionextract-minđược gọi. 
...
4. extract-min O(logn): xoá root node chứa minimum-key
   ...
   1. Gồm 3 bước:
      ...
      3. Bước 3: Gộp các tree mà có cùng bậc. Bước này sẽ lặp đi lặp lại cho đến khi không có 2 tree nào cùng bậc trong fibonacci heap.
...
1. Phân biệt các loại cây nhị phân:
   ...
   3. [Link tham khảo đầy đủ](https://www.quora.com/What-is-the-difference-between-complete-and-full-binary-trees)
...
4. Binomial heap: 
   1. [binomial heap brilliant](https://brilliant.org/wiki/binomial-heap/)
   2. [youtube](https://www.youtube.com/watch?v=7UQd9SYUoNk)
   3. [Binomial heap application](https://en.wikipedia.org/wiki/Binomial_heap#Applications)
   4. [Binomial Queue visualization](https://www.cs.usfca.edu/~galles/visualization/BinomialQueue.html)
5. Fibonacci Heap:
   1. [Wikipedia](https://en.wikipedia.org/wiki/Fibonacci_heap)
   2. [fibonacci-heap, brilliant](https://brilliant.org/wiki/fibonacci-heap/#citation-3)
   3. [fibonacci heap princetion.edu](https://www.cs.princeton.edu/~wayne/cs423/lectures/fibonacci-4up.pdf)
   4. [why a fibonacci heap called a fibonacci heap](https://stackoverflow.com/questions/14333314/why-is-a-fibonacci-heap-called-a-fibonacci-heap)
6. [Paring Heap](https://brilliant.org/wiki/pairing-heap/)
